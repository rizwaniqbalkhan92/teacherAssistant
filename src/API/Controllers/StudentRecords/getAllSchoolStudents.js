...
const getAllStudentsUnderTeacher = async (req, res) => {
...
const filterDataByUserId = (data, userId) => {
    const result = {};
      ...
      for (const grade in data[term]) {
          ...
          for (const studentId in data[term][grade]) {
              console.log("studentId===>>",studentId)
                if (data[term][grade][studentId].userId === userId) {
                    if (!result[term]) {
                        result[term] = {};
                    }
                    ...
                    // result[studentId] = data[term][grade][studentId];
...
module.exports = getAllStudentsUnderTeacher;

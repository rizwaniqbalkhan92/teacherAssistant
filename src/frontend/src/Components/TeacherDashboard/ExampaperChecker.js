...
import { extractStudentDetails, generatePDF, getUserId } from '../Constants/utils';
import NotesList from './ListNotes';
import { generateExamsResults } from '../Constants/excamApisCalling';

const ExampaperChecker = () => {
...
const [userId,setUserId]=useState('');
  ...
  const handleGenerateResults =async () => {
    try {
        ...
        const response = await (capturedImages.length > 0 
          ? generateExamsResults(capturedImages, true) 
          : generateExamsResults(uploadedFiles, false));
  ...
  const handleSaveResults = async() => {
      try{
          ...
          const {stdClass,stdId,userId,term,teacherName,subject}=extractStudentDetails(resut)
          console.log(resut)
          console.log("data====>data2",stdClass,stdId,userId,term,teacherName,subject)
        let data = JSON.stringify({
            "stdRecord":resut !==null ? resut :'' ,
            "userId": userId !==null ? userId :'' ,
            "stdClass":stdClass !==null ? stdClass :'' ,
            "term":term !==null ? term :'' ,
            "stdId":stdId !==null ? stdId :'' ,
            "teacherName":teacherName !==null ? teacherName :'' ,
            "subject":subject !==null ? subject :'' 
          });
          console.lo("config=====",data)
          let config = {
            method: 'post',
            maxBodyLength: Infinity,
            url: 'https://testbackend-rho.vercel.app/api/saveStudentRecords',
            headers: { 
...
.catch((error) => {
 ...
 <Typography 
      ...
      sx={{ 
        textAlign: 'center', 
        fontWeight: 'bold', 
        color: '#3f51b5', // Customize the color if needed
    ...
    {(capturedImages.length > 0 || uploadedFiles.length > 0) && (
      <Box sx={{ marginTop: 3 }}>
          ...
          sx={{ marginTop: 2 }}
          onClick={()=>{  setCapturedImages([]);setUploadedFiles([]); }}
        >
          Refresh
        </Button>
        <Typography variant="h6" gutterBottom>
...
export default ExampaperChecker;
